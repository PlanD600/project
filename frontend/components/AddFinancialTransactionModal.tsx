...
const AddFinancialTransactionModal: React.FC<AddFinancialTransactionModalProps> = (props) => {
  if (!props || !props.projects || props.projects.length === 0) return <div>Loading...</div>;
  try {
    ...
    const handleSubmit = (e: React.FormEvent) => {
      ...
      const selectedProject = props.projects.find(p => p.id === projectId) || props.projects[0];
    ...
    const title = props.type === 'Income' ? 'הוספת הכנסה' : 'הוספת הוצאה';
    const sourceLabel = props.type === 'Income' ? 'מקור הכנסה/לקוח' : 'שולם ל...';
    const canSelectProject = props.projects.length > 0;
    ...
    return <div className="text-danger">שגיאה בטעינת מודאל הוספת עסקה: {String(error)}</div>;
  }
}
